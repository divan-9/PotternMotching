...
/// <summary>
/// Adapts a non-nullable value-type pattern so it can be evaluated against a nullable value.
/// </summary>
/// <typeparam name="T">The non-nullable value type to match.</typeparam>
/// <param name="Inner">The pattern to evaluate when the actual value is not null.</param>
public sealed record NullableValuePattern<T>(
    IPattern<T> Inner) : IPattern<T?>
    where T : struct
{
    /// <inheritdoc />
        ...
        string path = "")
    {
        ...
        return Inner.Evaluate(value.Value, path);
    }
}

/// <summary>
/// A wrapper type for optional nullable nested value-type pattern matching with implicit conversions.
/// </summary>
/// <typeparam name="T">The non-nullable value type to match.</typeparam>
/// <typeparam name="TPatternDefault">The default non-null pattern implementation.</typeparam>
/// <remarks>
/// This type is used in generated pattern classes for nullable nested struct properties. When unspecified,
/// it matches any value. A nested pattern requires a non-null actual value, while null values can be
/// matched explicitly with <see cref="ValuePattern.Null"/> or an exact nullable value pattern.
/// </remarks>
public readonly struct NullableValuePatternDefault<T, TPatternDefault> : IPattern<T?>, IPatternConstructor<T?>
    where T : struct
    where TPatternDefault : IPattern<T>, IPatternConstructor<T>
{
    private readonly IPattern<T?>? innerPattern;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NullableValuePatternDefault{T, TPatternDefault}"/> struct.
    /// </summary>
    /// <param name="pattern">The inner nullable pattern to use for matching.</param>
    public NullableValuePatternDefault(
        IPattern<T?> pattern)
    {
        this.innerPattern = pattern;
    }
        ...
        T? value)
    {
        if (value is null)
        {
            return new NullableValuePatternDefault<T, TPatternDefault>(ValuePattern.Exact<T?>(null));
        }

        return new NullableValuePatternDefault<T, TPatternDefault>(
            new NullableValuePattern<T>(TPatternDefault.Create(value.Value)));
    }
    ...
    /// </summary>
    /// <param name="value">The value to convert.</param>
    public static implicit operator NullableValuePatternDefault<T, TPatternDefault>(
        T? value)
    {
        return (NullableValuePatternDefault<T, TPatternDefault>)From(value);
    }

    /// <summary>
    /// Implicitly converts a non-null value to a nullable pattern default.
    /// </summary>
    /// <param name="value">The value to convert.</param>
    public static implicit operator NullableValuePatternDefault<T, TPatternDefault>(
        T value)
    ...
    public static implicit operator NullableValuePatternDefault<T, TPatternDefault>(
        TPatternDefault pattern)
    {
        return new NullableValuePatternDefault<T, TPatternDefault>(new NullableValuePattern<T>(pattern));
    }
    ...
    public static implicit operator NullableValuePatternDefault<T, TPatternDefault>(
        ValuePattern<T?>.Exact pattern)
    {
        return new NullableValuePatternDefault<T, TPatternDefault>(pattern);
    }

    /// <summary>
    /// Implicitly converts an exact non-null value pattern to a nullable pattern default.
    /// </summary>
    /// <param name="pattern">The exact non-null value pattern to convert.</param>
    public static implicit operator NullableValuePatternDefault<T, TPatternDefault>(
        ValuePattern<T>.Exact pattern)
    {
        return new NullableValuePatternDefault<T, TPatternDefault>(new NullableValuePattern<T>(pattern));
